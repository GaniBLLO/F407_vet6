...
void static_host_configuration(uint8_t mac[6], uint8_t ip[4], uint8_t sn[4], uint8_t gw[4], uint8_t dns[4])
{
    ...
    USARTx_send_data("Setting up static host configuration...\r\n");

    // Set network configuration
    ctlnetwork(CN_SET_NETINFO, (void*)&net_info);

    USARTx_send_data("Static IP configuration is done successfully.\r\n");
...
void dynamic_host_configuration(uint8_t mac[6])
{
    ...
    USARTx_send_data("Setting up dynamic host configuration...\r\n");
    ...
    USARTx_send_data("Dynamic IP configuration is done successfully.\r\n");
}


void callback_ip_assigned()
{
	USARTx_send_data("DHCP: IP assigned\n");
    ip_assigned = true;
}

void callback_ip_renewed()
{
	USARTx_send_data("DHCP: IP renewed\n");
}

void callback_ip_conflict()
{
	USARTx_send_data("DHCP: IP conflict detected\n");
}
